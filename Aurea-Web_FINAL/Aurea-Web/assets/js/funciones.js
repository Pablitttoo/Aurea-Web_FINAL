...
function contacto() {
    ...
    if (contenido.length > 500) {
        mensaje.textContent = "El mensaje no puede superar los 500 caracteres.";
        mensaje.style.color = "red";
        return false;
    }

    let regexCorreo = /^[\w\.-]+@(duoc\.cl|profesor\.duoc\.cl|gmail\.com)$/;
    if (!regexCorreo.test(correo)) {
        mensaje.textContent = "El correo debe ser de dominio @duoc.cl, @profesor.duoc.cl o @gmail.com.";
        mensaje.style.color = "red";
        return false;
    }
    ...
    if (usuarioValido) {
        mensaje.textContent = "Gracias " + nombre + 
            " por contactarnos. Hemos recibido tu mensaje y te responderemos a la brevedad al correo: " + correo;
        mensaje.style.color = "green";
    } else {
        mensaje.textContent = "Correo no encontrado en nuestros registros.";
        mensaje.style.color = "red";
    }
